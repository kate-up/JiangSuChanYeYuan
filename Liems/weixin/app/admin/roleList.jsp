...
<head>
	...
	<script type="text/javascript"> 
		...
		function loadData(){
			aweto.ajax(basePath+namespace+"/tableList.ajax",{pageNo:pageNo,pageSize:5,comNo:comNo},true,function(r){
				pageNo=r.pageNo+1;
				...
				for (var i = 0; i < rows.length; i++) {
					...
					tmp.push('		<h5>'+row.roleName+'</h5>');
					tmp.push('		<h4>'+row.members+' <span>人</span></h4>');
					tmp.push('		<ul>');
					tmp.push('			<li>'+row.roleDesc+'</li>');
					tmp.push('		</ul>');
					tmp.push('		<button class="button z-depth-1" onclick="toRoleUserList('+row.row_pkValue+')">管理成员</button>');
					tmp.push('	</div>');
					tmp.push('</div>');
					tmp.push('</div>');
		...
		function toRoleUserList(pk){
			weixin.toRoleUserList(pk);
		}
	</script>
	<style type="text/css">
		.list-text{
			...
			<div class="row">
				<div class="col s9">
					<div class="content-left">
						<a href="javascript:weixin.toIndex()"><h1><i class="fa fa-chevron-left"></i><span></span>角色分配</h1></a>
					</div>
				</div>
				<div class="col s3" >
