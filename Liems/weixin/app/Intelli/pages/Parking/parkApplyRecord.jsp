...
<body class="gray-bg" :style="{height: screenHeight + 'px'}" >
    <div id="parkApply" class="cont">
        ...
        <div style="height: calc(100% - 94px);overflow-y: scroll;">
            <div class="each_list" v-for="(item, index) in lists" @click="toDet(item.applyNo)">
                <div class="first_row" >
                    <div style="color: #333;">{{item.comName}}</div>
<%--                    <div>{{item.visitorTyp}}</div>--%>
                </div>
                <div>姓名：{{item.usrNam}}</div>
                <div>联系电话：{{item.usrPhone}}</div>
                <div>申请时长：{{item.appleTime}}</div>
            </div>
        </div>
    </div>
</body>
<script type="text/javascript">
    ...
    var records=[];
    aweto.ajax(basePath + "/spm/spmApplyCar/getApplyCarListInfo.ajax", {type:0}, false, function (data) {
        if (data.result == "fail") {
    ...
    new Vue({
        ...
        data: {
            ...
            lists:records,
            // lists: [
            //     {visitorCony: "xxx企业", visitName: "陈丽丽", time:"一个月"},
            //     {visitorCony: "xxx企业", visitName: "陈丽丽", time:"一个月"},
            //     {visitorCony: "xxx企业", visitName: "陈丽丽", time:"一个月"}
            // ],
            index: 0
        ...
        methods: {
            toDet (applyNo) {
                if(this.index == 0){
                    window.location = basePath + "weixin/app/Intelli/pages/Parking/parkAudit.jsp?applyNo=" + applyNo;
                }else if(this.index == 1){
                    window.location = basePath + "weixin/app/Intelli/pages/Parking/parkCheckedInfo.jsp?applyNo=" + applyNo;
                }
            }
        },
        created(){
        ...
        obj.onclick = function () {
            for(var j=0; j<arrLi.length; j++) {
                if(obj == arrLi[j]) {
                    ...
                    aweto.ajax(basePath + "/spm/spmApplyCar/getApplyCarListInfo.ajax", {type:j}, false, function (data) {
                        if (data.result == "fail") {
                            layer.msg(data.msg, {icon: 5});//不开心
                            res = false;
                        }else{
